...
> "I'm about to write a specification for [your feature]. But first, let's assess together: What context have I loaded so far? Is it sufficient for a solid spec? What critical information am I missing that could lead to a vague specification? If YOU were writing this spec based on current context, what questions would you have? What's the difference between a specification written with our current context vs. one written with richer context? Help me understand WHEN we're ready to commit to a specification—not just WHAT to write."
